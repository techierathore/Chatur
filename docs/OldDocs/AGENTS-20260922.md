...
## Project basics
- Stack: .NET 10, MAUI Blazor Hybrid (Mac Catalyst + Windows), screens in the `ChaturUI` Razor class library using TrBlazeUI, behaviour in `Chatur.Core`, SQLite with Dapper + DbUp (no EF Core), TechieRag for model calls.
